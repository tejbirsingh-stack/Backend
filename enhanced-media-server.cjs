...
app.use(
  cors({
    origin: function(origin, callback) {
      ...
      // Allow localhost and 127.0.0.1 on various ports
      ...
      // Check if the origin is allowed or starts with file://
...
// Use absolute path to project root uploads directory
...
// Use the same resolved path as for static serving
...
async function scanUploadsFolder() {
  ...
  try {
    ...
    console.log(
      `📊 Found ${mediaAssets.length} media assets in uploads folder`
    );
    return mediaAssets;
  } catch (error) {
