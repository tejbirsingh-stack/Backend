...
async function sendUserInviteEmail({ request, user, roleName, orgName, planName, checkoutUrl, isRegistration = false }) {
  ...
  if (isRegistration) {
    emailSubject = `Welcome to Noah - Account Registered Successfully`;
    emailText = `Hello ${user.name || 'there'},\n\nYour account has been registered successfully by an administrator.\n\nEmail: ${user.email}\nRole: ${roleName || 'User'}\n\nPlease use the following link to set up your password and access your account:\n${setupUrl}\n\nBest regards,\nNoah Team`;
    emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
        <div style="text-align: center; margin-bottom: 24px;">
          <h2 style="color: #4f46e5; margin: 0; font-size: 24px;">Welcome to Noah!</h2>
        </div>
        ...
        <div style="background-color: #f8fafc; padding: 18px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #4f46e5;">
          ...
          <p style="margin: 0; color: #1e293b; text-transform: capitalize;"><strong>Role:</strong> ${roleName || 'User'}</p>
        </div>
        <p style="color: #334155; font-size: 15px;">To get started, please click the button below to set up your password:</p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${setupUrl}" style="background-color: #4f46e5; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 15px; display: inline-block; box-shadow: 0 4px 6px -1px rgba(79, 70, 229, 0.2);">Set Up Password</a>
        </div>
        <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 30px 0;" />
        <p style="font-size: 12px; color: #94a3b8; text-align: center;">This setup link will expire in 24 hours. If you have any questions, please reach out to your administrator.</p>
      </div>
    `;
  } else {
    emailSubject = isPaidPlan
      ? `Welcome to Noah - Complete ${orgName || 'Organization'} Setup & Subscription`
      : `Welcome to Noah - Complete Your ${orgName || 'Organization'} Setup`;

    emailText = isPaidPlan
